...
class ContourMapView(TabView):
  ...
  def load_view(self):
    ...
    n_curves_entry = self.__create_entry_row(form_entries_frame, label_text)
  ...
  def __start_creation_chart(
    ...
    try:
      ...
      self.chart_builder = level_chart.ContourMapBuilder(dataset=dataset)
  ...
  def __generate_animated_chart(
    ...
    print(f'-> ContourMap gif for "{variable}" variable.')
    dim_constraints = {
      'depth': [depth],
